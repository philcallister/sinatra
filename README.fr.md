...
Sinatra n'impose pas de modèle de la concurrence. Sinatra est thread-safe, vous pouvez
donc utiliser n'importe quel gestionnaire Rack, comme Thin, Puma ou WEBrick en mode
multi-threaded.

Lorsque le serveur est démarré, vous devez spécifier l'appel de méthode correcte pour
chaque gestionnaire Rack particulier.

L'exemple ci-dessous montre comment vous pouvez exécuter un server Thin de manière
multi-threaded:
